...
export const registerAskTool = (server: McpServer, client: ZephApiClient, config: McpServerConfig, waiter?: HookResponseWaiter) => {
  server.registerTool(
    ...
    {
      description:
        'Ask the user a question on their phone: quick-reply buttons plus a text field — they tap or type. Pass `actions` (2–4 buttons) unless the answer is free-form text. Blocks until they answer or `timeout` passes. Requires ZEPH_HOOK_ID. Files the user attaches arrive as local absolute paths in the result\'s `attachments`; reading them is part of the answer. Never end-to-end encrypted (unlike zeph_notify / zeph_file), so keep secrets out of the question and do not expect a private answer.',
      annotations: {
        readOnlyHint: false,
